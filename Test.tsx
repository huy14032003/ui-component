...
const tableData: Person[] = Array.from({ length: 45 }).map((_, i) => ({
  id: `USR-${i + 1}`,
  name: `Nhân viên ${i + 1}`,
  age: 20 + (i % 15),
...
const tableColumns: ColumnDef<Person>[] = [
  ...
  },
  {
    ...
    cell: info => {
      ...
      return <span className="text-gray-600">{role}</span>;
    }
  },
  {
    accessorKey: 'status',
    ...
    cell: info => {
      ...
      if (status === 'active') return <Tag variant="ghostSuccess">Hoạt động</Tag>;
      if (status === 'inactive') return <Tag variant="ghostDanger">Đã khoá</Tag>;
      return (
        <Tag variant="ghostWarning">Chờ duyệt</Tag>
      );
    }
  },
  {
    ...
    header: 'Hành động',
    enableSorting: false,
    cell: ({ row, column, cell, getValue, table, renderValue }) => {
      const person = row.original;
      return (
        <div className="flex items-center gap-2">
          <Button size="xs" variant="ghostPrimary" onClick={() => alert(`Sửa nhân viên: ${person.name}`)}>Sửa</Button>
          <Button size="xs" variant="ghostDanger" onClick={() => alert(`Khoá nhân viên: ${person.name}`)}>Khoá</Button>
        </div>
      );
    }
  },
  {
    ...
    cell: (props) => {
      return (
        <div className="flex items-center gap-2">
          <Button size="xs" variant="primary" onClick={() => console.log(props)}>thông tin props</Button>
        </div>
      );
    }
  }
...
const formSchema = z.object({
  ...
  dateOfBirth: z.any().refine((val) => val !== undefined && val !== null, "Vui lòng chọn ngày sinh"),
  bio: z.string().min(10, "Tiểu sử phải có ít nhất 10 ký tự"),
  terms: z.boolean().refine((val) => val === true, "Bạn phải đồng ý với điều khoản"),
  avatar: z.array(z.any()).min(1, "Vui lòng tải lên ảnh đại diện")
});
...
const Test = () => {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    alert("Dữ liệu hợp lệ! Đã log ra console.");
  };

  const tourSteps: { target: string, title: string, description: string, placement: "top" | "bottom" | "left" | "right" }[] = [
    {
      target: '.tour-step-1',//className
      title: 'Khu vực Tour',
      description: 'Đây là khu vực ví dụ cho tính năng điểm nhấn hướng dẫn người dùng.',
      placement: 'bottom',
    },
    {
      target: '.tour-step-2',
      title: 'Bắt đầu',
      description: 'Nhấn vào nút này để bắt đầu trình diễn.',
      placement: 'left',
    },
    {
      target: '.tour-step-3',
      title: 'Ví dụ Avatar',
      description: 'Đây là ví dụ về nhóm Avatar chồng lên nhau.',
      placement: 'top',
    },
    {
      target: '.tour-step-4',
      title: 'Ví dụ Timeline',
      description: 'Đây là ví dụ về Timeline.',
      placement: 'top',
    }
  ];
...
<InputPassword label='Input password' placeholder='Nhập mật khẩu' />`}</code>
      ...
      <p className='text-xl font-bold mt-8 mb-4 border-b pb-2'>11. Avatar</p>
      <div className="space-y-6 p-4">
        <div className="flex flex-wrap gap-4 items-center">
          <Avatar size="sm" alt="User 1" />
          <Avatar size="md" alt="User 2" src="https://i.pravatar.cc/150?u=1" />
          <Avatar size="lg" alt="User 3" fallback="AB" />
          <Avatar shape="square" size="md" fallback="SQ" />
        </div>

        <h3 className="font-semibold text-lg text-gray-700">Avatar Group</h3>
        <AvatarGroup max={3} size="md" className="tour-step-3">
      ...
      <p className='text-xl font-bold mt-8 mb-4 border-b pb-2 tour-step-4'>13. Timeline</p>
      <div className="p-4">
        <Timeline>
          <TimelineItem color="success">
            <p className="font-semibold text-gray-900">Tạo dự án</p>
            <p className="text-sm text-gray-500">20/03/2026</p>
          </TimelineItem>
          <TimelineItem color="primary">
            <p className="font-semibold text-gray-900">Thiết kế hệ thống giao diện</p>
            <p className="text-sm text-gray-500">21/03/2026</p>
          </TimelineItem>
          <TimelineItem color="warning">
            <p className="font-semibold text-gray-900">Triển khai code</p>
            <p className="text-sm text-gray-500">Đang thực hiện...</p>
          </TimelineItem>
          <TimelineItem color="gray">
            <p className="font-semibold text-gray-900">Kiểm thử và phát hành</p>
            <p className="text-sm text-gray-500">Chưa bắt đầu</p>
          </TimelineItem>
        </Timeline>
      </div>

      <p className='text-xl font-bold mt-8 mb-4 border-b pb-2'>14. Tour</p>
      <div className="p-4 border border-dashed border-gray-300 rounded-lg flex items-center justify-between tour-step-1">
        <div>
          <h3 className="font-semibold text-gray-900">Trải nghiệm Tour</h3>
          <p className="text-sm text-gray-500">Nhấn nút bên cạnh để bắt đầu hướng dẫn.</p>
        </div>
        <Button variant="primary" onClick={() => setIsTourOpen(true)} className="tour-step-2">Bắt đầu Tour</Button>
      </div>
      <Tour open={isTourOpen} onClose={() => setIsTourOpen(false)} steps={tourSteps} />

      <p className='text-xl font-bold mt-8 mb-4 border-b pb-2'>15. Table (TanStack)</p>
      <div className="space-y-8 p-4 bg-gray-50 rounded-xl">

        <div>
          <h3 className="font-semibold text-lg text-gray-700 mb-3">15.1 Bảng cơ bản (Basic Table)</h3>
          <Table
            data={tableData}
            columns={tableColumns}
            enablePagination={true}
            enableSorting={true}
            // Tuỳ chỉnh text chọn size
            renderPageSizeText={(size) => `${size} / trang`}


            // Tuỳ chỉnh text nhảy trang
            // goToPageText={false}
            goToPageText={"Đi đến trang"}


            // Tuỳ chỉnh text hiển thị
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-lg text-gray-700">15.2 Bảng Đa năng (Sorting, Pagination, Checkbox)</h3>
            <Button size="sm" variant="outlinePrimary" onClick={() => console.log('Tải lại dữ liệu')}>Tải lại</Button>
          </div>
          <Table
            data={tableData}
            columns={tableColumns}
            ...
            onSelectionChange={(selected) => console.log('Đã chọn:', selected)}
          />
        </div>

        <div>
          <h3 className="font-semibold text-lg text-gray-700 mb-3">15.3 Trạng thái Tải dữ liệu (Loading)</h3>
          <Table
        ...
        <div>
          <h3 className="font-semibold text-lg text-gray-700 mb-3">15.4 Trạng thái Trống (Empty State)</h3>
          <Table
            data={[]}
            columns={tableColumns}
            enablePagination={false}
        ...
        <div>
          <h3 className="font-semibold text-lg text-gray-700 mb-3">15.5 Bảng Mở rộng dòng (Expandable Rows)</h3>
          <Table
            data={tableData.slice(0, 5)}
            columns={tableColumns}
            enablePagination={false}
            ...
            getRowCanExpand={(row) => row.role === 'Admin'}
            renderSubComponent={({ row }) => (
              <div className="py-2">
                <h4 className="font-semibold text-primary mb-2">Thông tin chi tiết nhân viên: {row.name}</h4>
                <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                  <p><span className="font-medium text-gray-700">Mã định danh:</span> <span className="text-gray-500">{row.id}</span></p>
                  <p><span className="font-medium text-gray-700">Độ tuổi:</span> <span className="text-gray-500">{row.age}</span></p>
                  <p><span className="font-medium text-gray-700">Chức vụ:</span> <span className="text-gray-500">{row.role}</span></p>
                  <p><span className="font-medium text-gray-700">Tình trạng:</span> <span className="text-gray-500">{row.status}</span></p>
                </div>
                <p className="text-gray-600 text-sm leading-relaxed italic">
                  * Ghi chú: Đây là dữ liệu động được truy xuất trực tiếp từ thực thể <strong>{row.name}</strong>.
                  Bạn có thể tùy biến giao diện này cho riêng từng loại nhân viên dựa trên các điều kiện logic khác nhau.
                </p>
              </div>
            )}
          />
      ...
      <p className='text-xl font-bold mt-8 mb-4 border-b pb-2'>16. Form Validation (React Hook Form + Zod)</p>
      <div className="p-6 bg-gray-50 rounded-xl mb-20 max-w-3xl">
        <FormProvider {...formMethods}>
          <form onSubmit={formMethods.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FormInput name="username" label="Tên đăng nhập" placeholder="Nhập tên đăng nhập..." />
              <FormInput name="password" label="Mật khẩu" placeholder="Nhập mật khẩu" isPassword={true} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FormInputDate name="dateOfBirth" label="Ngày sinh" variant="primary" valueFormat="DD/MM/YYYY HH:mm:ss" showTime="second" />
              <FormRadioGroup name="gender" label="Giới tính" orientation="horizontal">
            ...
            <FormTextarea name="bio" label="Tiểu sử" placeholder="Giới thiệu một chút về bạn..." />

            <FormUpload name="avatar" label="Ảnh đại diện" multiple={true} accept="image/*" />

            <div className="mt-4">
              <FormCheckbox name="terms" label="Tôi đồng ý với các điều khoản bảo mật" />
            </div>

            <div className="flex justify-end pt-4 border-t border-gray-200">
              <Button type="button" onClick={formMethods.handleSubmit(onSubmit)} variant="primary" size="lg">Đăng ký thành viên</Button>
            </div>
          </form>
        </FormProvider>
      </div>
